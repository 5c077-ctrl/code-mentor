...
export default function CourseDetailClientView({ course }: { course: any }) {
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '2.5rem' }}>
      ...
      >
        ...
        >
          ...
          <span style={{ color: '#ef4444', fontSize: '0.875rem', fontWeight: 700, display: 'flex', alignItems: 'center', gap: '0.3rem' }}>
            <span style={{ fontSize: '1.25rem', lineHeight: 0 }}>•</span> {totalLessons} {t('lecturesCount')}
          </span>
        </div>
        ...
        >
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', color: 'var(--text-secondary)' }}>
            ...
            <span>{totalLessons} {t('sublessons')}</span>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', color: 'var(--text-secondary)' }}>
        ...
        {firstLesson && (
          <Link href={`/learn/${course.slug}/${firstLesson.slug}`}>
            <Button size="lg">{t('startLesson')} ({totalLessons} {t('lecturesCount')})</Button>
          </Link>
        )}
      </section>
      ...
      >
        ...
        <section>
          ...
          >
            <BookOpen size={24} color="var(--accent-primary)" /> {t('modules')} & {t('sublessons')}
          </h2>
              ...
              <Card key={mod.id} style={{ padding: '1.5rem' }}>
                ...
                >
                  <h3 style={{ fontSize: '1.25rem', fontWeight: 700, margin: 0 }}>
                    <span style={{ color: 'var(--accent-primary)', marginRight: '0.6rem', fontWeight: 800 }}>
                      {String(modIdx + 1).padStart(2, '0')}
                    </span>
                    {mod.title}
                  </h3>
                  <Badge variant="default">
                    {mod.lessons.length} {t('sublessons')}
                  </Badge>
                </div>
                    ...
                    return (
                      ...
                      >
                        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                          <div style={{ fontSize: '1.25rem', fontWeight: 800, color: 'var(--accent-primary)', minWidth: '32px', fontFamily: 'monospace' }}>
                            {paddedNum}
                          </div>
                          <div>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', color: 'var(--accent-primary)' }}>
                          <PlayCircle size={18} />
                          <span style={{ fontSize: '0.8rem', fontWeight: 600 }}>{t('startLesson')}</span>
                        </div>
                      </Link>
                    );
                  })}
        ...
        >
          <Card>
            <h3 style={{ fontSize: '1.1rem', marginBottom: '1rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              <Wrench size={18} color="var(--accent-warning)" /> {t('courseCatalog')}
            </h3>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', color: 'var(--text-secondary)', fontSize: '0.9rem' }}>
                <span>{t('lecturesCount')}</span>
                <span style={{ color: '#ef4444', fontWeight: 700 }}>
                  {totalLessons} {t('lecturesCount')}
                </span>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', color: 'var(--text-secondary)', fontSize: '0.9rem' }}>
